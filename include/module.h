...
//------------------------------------------------------------------------------------------------
// обязательный параметр конфигурации для всех модулей
//------------------------------------------------------------------------------------------------
...
// Данные с модул
typedef struct __attribute__((packed)) {
  float voltage_vdd; //  напряжение питания проца (опорное напряжение ацп)
  float current_24v; //  ток по личнии 24 вольта
  float voltage_24v; //  напряжение по линии 24 вольта
  float voltage_5v_ext; //  напряжение по линии 5 вольт для внешних устройств
  float    voltage_5v; //  напряжение по линии 5 вольт
  bool     ac_detect;  //  флаг состояния общей сети
  bool     relay0_sts; //  состояние реле
  bool     relay1_sts; //  состояние реле
  bool     red_led;    //  состояние светодиода
  bool     green_led;  //  состояние светодиода
  uint16_t timestamp;  //  время события
} psu_module_data_t;

//  Данные в модуль
typedef struct {
  uint8_t relay0; //  установка состояния реле, 1-on / 0-off
  uint8_t relay1; //  установка состояния реле, 1-on / 0-off
} psu_module_set_data_t;
